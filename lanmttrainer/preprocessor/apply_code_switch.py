"""
使用Code Switch技术对语料进行增强
TODO 由于对其工具的限制，目前只支持中文到其他语言的对齐
"""
import os
import typer
...
def apply_code_switch(
    ...
    translation_table: str = typer.Option(..., "--translation-table", "-t", help="翻译表位置"),
    alignment_table: str = typer.Option(...,
                                        "--alignment-table", "-a", help="对齐表位置")
):
    files = [f for f in os.listdir(folder) if os.path.isfile(os.path.join(folder, f))]
    if len(files) != 2 or "zh" not in files:
        typer.echo(f"{folder} 不是一个合法的语料文件夹，请检查后重试。必须包含两个文件，以语种简称命名：如`zh, en`",
                   err=True, color="red")
        typer.Exit(1)
