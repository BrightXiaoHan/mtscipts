...
def shared_large_datasets(
    ...
    for pair in lang_pairs:
        ...
        srcfile = data_dir / f"{trainpref}.{pair}.{src_lang}"

        for i, chunk in enumerate(divide(num_epoch, srcfile.open())):
            outfile = data_dir / f"part{i}.{trainpref}.{pair}.{src_lang}"
            with open(outfile, "w") as fout:
                for line in chunk:
                    fout.write(line)
        tgtfile = data_dir / f"{trainpref}.{pair}.{tgt_lang}"

        for i, chunk in enumerate(divide(num_epoch, tgtfile.open())):
            outfile = data_dir / f"part{i}.{trainpref}.{pair}.{tgt_lang}"
            with open(outfile, "w") as fout:
                for line in chunk:
                    fout.write(line)
