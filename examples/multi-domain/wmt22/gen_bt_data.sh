...
mkdir -p $TRAIN_DIR/data-bin-${SRCLANG}-${TGTLANG}/monolingual-${SRCLANG}
fairseq-preprocess --source-lang ${SRCLANG}_RWSK --target-lang ${TGTLANG}_RWSK \
    --testpref $DATA_DIR/wmtnews/monolingual.${SRCLANG} \
    --srcdict $TRAIN_DIR/data/fairseq.vocab \
    --tgtdict $TRAIN_DIR/data/fairseq.vocab \
    --destdir $TRAIN_DIR/data-bin-${SRCLANG}-${TGTLANG}/monolingual-${SRCLANG} \
    --workers 30 > $TRAIN_DIR/data-bin-${SRCLANG}-${TGTLANG}/monolingual-${SRCLANG}/preprocess.log 2>&1
...
fairseq-generate $TRAIN_DIR/data-bin-${SRCLANG}-${TGTLANG}/monolingual-${SRCLANG} \
  --path $pretrained_model --fp16 \
  --task translation_multi_simple_epoch \
  --gen-subset test \
  --source-lang ${SRCLANG}_RWSK \
  --target-lang ${TGTLANG}_RWSK \
  --lang-dict "$lang_list" \
  ...
  > $TRAIN_DIR/data-bin-${SRCLANG}-${TGTLANG}/monolingual-${SRCLANG}/btdata.out
